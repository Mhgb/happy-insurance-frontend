export const URL = "http://localhost:8080";
// export const URL = "https://happy-insurance-latest.onrender.com";

export const insuranceInfo = [
  {
    id: 1,
    classname: "health-insurance-info",
    topic: "Health Insurance",
    def: "A health insurance or a medical insurance is a type of general insurance that safeguards you against financial losses by covering for you when you’re faced with a health condition or medical emergency such as due to a disease, illness or even an accident. Insurance that covers the medical expenses of the insured due to an illness or accident in exchange for a premium amount. It enables the insurance company to provide medical coverage for hospitalization expenses, day care procedures, critical illnesses, etc",
  },
  {
    id: 2,
    classname: "vehicle-insurance-info",
    topic: "Vehicle Insurance",
    def: "An insurance that protects you and your vehicle (car/bike/commercial vehicle) from financial losses and damages like natural calamities, thefts, burglaries, malicious acts and accidents. Motor insurance is just like any other insurance policy, but unlike other insurances, it is ‘Mandatory'! And, as the name suggests, it is an insurance that is related to all types of motor vehicles-motorcycles, cars, jeeps, commercial vehicles etc. vehicle insurance, is a mandatory policy designed for four-wheelers, trucks, two-wheelers, and other vehicles plying on the roads of India",
  },
  {
    id: 3,
    classname: "term-insurance-info",
    topic: "Term Insurance",
    def: "Term life insurance is a type of life insurance policy that provides coverage for a certain period of time, or a specified “term” of years. If the insured dies during the time period specified and the policy is active, or “in force,” then a death benefit will be paid.",
  },
];

export const featuredPosts = [
  {
    id: 1,
    topic:
      "Why Life Insurance Should Be Part of Every Entrepreneur’s Business Plan",
    content:
      "As an entrepreneur, you already know that many risks and uncertainties come with running a business. However, there is one crucial element that often goes overlooked in many entrepreneurs’ business plans: life insurance.",
  },
];

export const morePosts = [
  {
    id: 1,
    topic: "Meet Sirin Thada, Artist in Residence for Life Happens",
    content:
      "We partnered with Thai-American artist Sirin Thada to create illustrations that reflect the theme, Life Insurance: For Anyone Who Lives.",
  },
  {
    id: 2,
    topic: "Celebrating 20 Years, 20 Moments of Life Insurance Awareness Month",
    content:
      "Life Happens’ 20th annual Life Insurance Awareness Month (LIAM) is officially here! This September marks the 20th anniversary of our biggest campaign, and we’re celebrating all month long.",
  },
  {
    id: 3,
    topic: "Celebrating 20 Years, 20 Moments of Life Insurance Awareness Month",
    content:
      "Life Happens’ 20th annual Life Insurance Awareness Month (LIAM) is officially here! This September marks the 20th anniversary of our biggest campaign, and we’re celebrating all month long.",
  },
  {
    id: 4,
    topic:
      "Upgrading Your Life Insurance: Is Switching from Term to Perm Right for You?",
    content:
      "If you currently have a term life insurance policy, you may be wondering if it's possible to make the switch to permanent life insurance. The good news is you can, but there are some important factors to consider first.",
  },
  {
    id: 5,
    topic:
      "Participate in Life Happens’ Facebook Chat for Life Insurance Awareness Month",
    content:
      "Join Life Happens for a Facebook Chat during Life Insurance Awareness Month this September. We’ll discuss all things life insurance and living benefits!",
  },
  {
    id: 6,
    topic: "What Women Need to Know About Life Insurance",
    content:
      "If someone depends on you, chances are you need life insurance. Why, then, do fewer women own life insurance than men? There are some misconceptions that prevent women from getting the coverage they need.",
  },
  {
    id: 7,
    topic: "What Women Need to Know About Life Insurance",
    content:
      "If someone depends on you, chances are you need life insurance. Why, then, do fewer women own life insurance than men? There are some misconceptions that prevent women from getting the coverage they need.",
  },
  {
    id: 8,
    topic:
      "5 Advantages of Combining Annuities and Life Insurance for Retirement",
    content:
      "Using both annuities and life insurance as part of your retirement strategy can give you income during your retired years, as well as a death benefit after you die.",
  },
  {
    id: 9,
    topic: "I Just Graduated. Isn’t It Too Early to Talk About Life Insurance?",
    content:
      "Life insurance may not seem like a priority right now, especially with competing financial concerns. But being a young adult means you’re entering a new stage of life. Here are a few reasons to consider purchasing life insurance now as a college graduate.",
  },
];
